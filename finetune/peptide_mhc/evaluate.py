...
and ROC curves on held-out test peptides (saved by train.py).

Usage:
    from finetune.peptide_mhc.evaluate import evaluate

    evaluate({
        "attn":        "results/finetune/peptide_mhc/attn/best_model.pt",
        "modern_attn": "results/finetune/peptide_mhc/modern_attn/best_model.pt",
    })

    # or run directly:
    python main.py finetune --task peptide_mhc --eval
"""
...
def evaluate(models: dict[str, str], device: str = None):
    ...
    if not os.path.exists(CFG["s2_test_split_path"]):
        raise FileNotFoundError(
            f"Test split not found at {CFG['s2_test_split_path']}. Run train.py first."
        )

    test_data    = torch.load(CFG["s2_test_split_path"], map_location="cpu")
    mhc_tokens   = test_data["mhc_tokens"]
    test_pep_idx = test_data["test_pep_idx"]
    pos_dict     = test_data["pos_dict"]
    ...
    if cached.get("peptide_list") != test_data.get("peptide_list"):
        raise RuntimeError(
            ...
            "Delete the cache and re-run train.py."
        )

    test_pep_embs = pep_embs[test_pep_idx]
    ...
    _save_bar_plot(results, k_values, metric="recall",
                   ...
                   ylabel="Mean Recall",
                   out_path=os.path.join(TASK_RESULTS_DIR, "recall_at_k.png"))
    _save_bar_plot(results, k_values, metric="precision",
                   ...
                   out_path=os.path.join(TASK_RESULTS_DIR, "precision_at_k.png"))
    _save_roc_plot(results,
                   out_path=os.path.join(TASK_RESULTS_DIR, "roc_all_models.png"))
    return results
...
def run():
    ...
    # Only evaluate checkpoints that have been trained
