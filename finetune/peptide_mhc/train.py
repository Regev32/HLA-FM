...
Models trained:
  attn        — classic MAE pretrained backbone, last k layers unfrozen
  modern_attn — modern MAE pretrained backbone, last k layers unfrozen
                (only when compare_architectures=true in config)

Usage (from project root):
    python main.py finetune --task peptide_mhc
"""
...
TASK_RESULTS_DIR = os.path.join(CFG["finetune_results_dir"], "peptide_mhc")
...
def build_encoder(params: dict, device: str,
                  ...
                  arch: str = "classic") -> nn.Module:
    """
    arch: "classic" → MHCAttentionEncoder
          "modern"  → ModernMHCEncoder
    """
    ...
    def _make_encoder(extra_kwargs=None):
        kw = {**base_kwargs, **(extra_kwargs or {})}
        if arch == "modern":
            classic_ref = MHCAttentionEncoder(**{k: v for k, v in kw.items()
                                                 if k not in ("use_moe","num_experts","top_k","ffn_hidden_dim")})
            target_n = sum(p.numel() for p in classic_ref.parameters())
            ffn_h = find_matching_hidden_dim(target_n, {k: v for k, v in kw.items()
                                                         if k != "ffn_hidden_dim"})
            kw["ffn_hidden_dim"] = ffn_h
    ...
    if pretrained and os.path.exists(mae_ckpt_path):
        ...
        saved_arch = ckpt["arch"]
        encoder = _make_encoder({
            "embed_dim":  saved_arch["embed_dim"],
            "nhead":      saved_arch["nhead"],
            "num_layers": saved_arch["num_layers"],
            "dropout":    params.get("dropout", saved_arch["dropout"]),
        })
        ...
        if pretrained:
            ...
            print(f"  {arch} random init backbone")
        encoder = _make_encoder()
...
def retrain_best(params, pretrained, unfreeze_layers,
                 ...
                 train_pos, val_pos, device, results_dir, arch="classic"):
...
def train_model(results_name, pretrained, unfreeze_layers,
                mhc_tokens, pep_embs, train_pep_idx, val_pep_idx,
                train_pos, val_pos, device, arch="classic"):
    ...
    train_losses, val_losses = retrain_best(
        ...
        mhc_tokens, pep_embs, train_pep_idx, val_pep_idx,
        train_pos, val_pos, device, results_dir, arch=arch,
    )
...
def run():
    ...
    os.makedirs(os.path.dirname(CFG["s2_test_split_path"]), exist_ok=True)
    ...
    all_losses["attn"] = train_model("attn", pretrained=True,
                unfreeze_layers=CFG["s2_unfreeze_layers"], **shared, arch="classic")

    if CFG.get("compare_architectures", False):
        print(f"\n{'='*55}\n  Model: modern_attn  (modern pretrained)\n{'='*55}")
        all_losses["modern_attn"] = train_model("modern_attn", pretrained=True,
                    unfreeze_layers=CFG["s2_unfreeze_layers"], **shared, arch="modern")

    os.makedirs(TASK_RESULTS_DIR, exist_ok=True)
    _save_combined_loss_plot(all_losses,
                             os.path.join(TASK_RESULTS_DIR, "models_loss.png"))
