...
amino acid prediction. Run this FIRST, before peptide_mhc.py.
...
def main():
    ...
    torch.save({"train": train_idx.tolist(), "val": val_idx.tolist(), "test": test_idx.tolist()}, CFG["s1_split_path"])
    ...
    best_weights["mae_decoder"] = copy.deepcopy(decoder.state_dict())
    ...
    ax.set_xlim(left=10)
    clipped = train_losses[9:] + val_losses[9:]
    if clipped:
        ax.set_ylim(min(clipped), max(clipped))
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss (CE)")
    ax.set_title("Pretraining loss- train & val loss VS. epochs")
    ax.legend()
    fig.savefig(os.path.join(output_dir, "pretrain_loss.png"), dpi=300)
    plt.close(fig)
