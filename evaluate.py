...
and ROC curves on the held-out test MHC alleles (saved by train_mhc.py).

Usage:
    from evaluate import evaluate

    evaluate({
        "pretrained":  "results/attn/best_model.pt",
        "random_init": "results/attn_baseline/best_model.pt",
    })
...
def _load_checkpoint(ckpt_path: str, device: str):
    ...
    pep_proj = nn.Linear(CFG["s2_peptide_dim"], output_dim).to(device)
...
def _compute_roc_curve(scores: torch.Tensor, pos_dict: dict,
    ...
    negatives per allele so FPR is not dominated by the full 425k negative pool.
    ...
    for i, pos_set in pos_dict.items():
        ...
        # Sample negatives: boolean mask is fast even at 425k
        ...
        all_aucs.append(float(np.trapz(tpr, fpr)))
...
def evaluate(models: dict[str, str], device: str = None):
    """
    Args:
        models: {name: checkpoint_path} — any number of stage 2 checkpoints
        device: torch device string. Auto-detected if None.

    Returns:
        dict {name: {"recall": [...], "precision": [...]}}
    """
    if device is None:
        device = (
    ...
    if not os.path.exists(CFG["s2_test_split_path"]):
        raise FileNotFoundError(
            f"Test split not found at {CFG['s2_test_split_path']}. Run train_mhc.py first."
        )

    test_data   = torch.load(CFG["s2_test_split_path"], map_location="cpu")
    test_tokens = test_data["mhc_tokens"]   # (N_test, seq_len)
    pos_dict    = test_data["pos_dict"]     # local_mhc_idx → set of pep_idx
    ...
    if cached.get("peptide_list") != test_data.get("peptide_list"):
        raise RuntimeError(
            ...
            "Delete the cache and re-run train_mhc.py."
        )
    n_pep    = len(pep_embs)
    ...
    for name, ckpt_path in models.items():
        ...
        # Encode all test MHCs
        with torch.no_grad():
            mhc_emb = F.normalize(encoder(test_tokens.to(device)), dim=-1)  # (N_test, D)

        # Compute scores in batches over peptides to keep memory manageable
        scores = torch.zeros(len(test_tokens), n_pep)
        batch  = CFG["s2_peptides_per_batch"]

        with torch.no_grad():
            for start in range(0, n_pep, batch):
                pep_batch = F.normalize(
                    pep_proj(pep_embs[start : start + batch].to(device)), dim=-1
                )
                scores[:, start : start + batch] = (mhc_emb @ pep_batch.T).cpu()

        recalls_per_k, precisions_per_k = [], []
        for k in k_values:
            valid      = [(i, ps) for i, ps in pos_dict.items() if len(ps) > 0]
            ...
            print(f"  Recall@{k:<5} = {mean_recall:.4f}  "
                  ...
                  f"(over {len(valid)} test alleles)")

        fpr_grid, mean_tpr, mean_auc = _compute_roc_curve(scores, pos_dict)
...
def _save_roc_plot(results, out_path):
    ...
    ax.set_title("ROC Curve on held-out test MHC alleles\n"
                 "(negatives sampled at 10× positives per allele)")
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.legend()
...
if __name__ == "__main__":
    evaluate({
        "pretrained":  os.path.join(CFG["results_dir"], "attn",          "best_model.pt"),
        "random_init": os.path.join(CFG["results_dir"], "attn_baseline", "best_model.pt"),
        "probe":       os.path.join(CFG["results_dir"], "attn_probe",    "best_model.pt"),
    })
